...
const Video = () => {
  ...
  const loadSong = async () => {
    await axios
    .get(`http://localhost:8000/api/v1/videos/${id}`)
    .then((res) => {
      console.log(res.data);
      // setData(res.data.findVideo);
    })
      ...
      <div className="left flex-1">
          ...
          <div className="flex items-center">
            ...
            <div className="px-3">
              <h3 className="text-yt-white font-medium text-base">
                {data?.channel_id && data?.channel_id.length <= 25
                  ? data?.channel_id
                  : `${data?.channel_id && data?.channel_id.substr(0, 20)}...`}
              </h3>
        ...
        <div className="max-w-4xl bg-yt-light-black mt-4 rounded-2xl text-sm p-3 text-yt-white">
          <div className="flex">
            ...
            <p className="font-medium pr-3">{uploadTime}</p>
          </div>
